...
config readConfig(const string& fileName)
{
	...
	for (int i = 0; i < lines.size(); i++)
	{
		...
		else if (tempTag == "(Fitness Function) Original/Constraint Satisfaction (o/c)")
		{
			if (tempString == "o")
				tempConfig.fitnessFunction = 1;
			else if(tempString == "c")
				tempConfig.fitnessFunction = 2;
			else
			{
		...
		else if (tempTag == "(Fitness) Penalty Coefficient (0.00 - infinity)")
			tempConfig.penaltyCoefficient = stof(tempString.c_str());
...
{
	int numBulbsAdj, tempInvalids = luBox.invalids;
	string tempString;
			...
			if (luBox.box[x][y] >= '0' && luBox.box[x][y] <= '4')
			{
				...
				if (luBox.box[x][y] == '1' && numBulbsAdj != 1)
				{
					if(constraintChecker)
					{
						...
						luBox.invalids += abs(stoi(tempString) - numBulbsAdj);
					}
					else
						return false;
				}	
	...
	if(tempInvalids < luBox.invalids)
		return false;

	return true;
}

void constraintFitness(lightUpBox & luBox, const config configSettings)
{
	luBox.invalids = 0;
	...
	if(!(adjacencyChecker(luBox, true)) && configSettings.adjacencyReq)
		luBox.fitness = 0;

	luBox.penaltyFitness = luBox.fitness - (luBox.invalids * configSettings.penaltyCoefficient);
...
void fitnessEvaluation(lightUpBox & luBox, const config configSettings)
{
	...
	if(configSettings.fitnessFunction == 2)
		constraintFitness(luBox, configSettings);
	else if(configSettings.adjacencyReq)
		if(!(adjacencyChecker(luBox, false)))
			luBox.fitness = 0;
...
void setLogHeader(ofstream & logOutput, const config configSettings)
{
	...
	if(configSettings.fitnessFunction == 1)
		logOutput << "Fitness Function: Original Fitness Function" << endl;
	else
	{
		logOutput << "Fitness Function: Constraint Satisfaction Fitness Function" << endl;
		logOutput << "Penalty Coefficient: " << configSettings.penaltyCoefficient << endl;
	}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < configSettings.numRuns; i++)
	{
		...
		if(configSettings.fitnessFunction == 1)
			...
			logOutput << numFitEvals << "\t" << averageFitness << "\t" << runBestBox.penaltyFitness << "\t" << runBestBox.fitness << endl;
		...
		do
		{
			...
			if(configSettings.parentSelection == 1)
				parents = fitnessProportionalSelection(population, configSettings, true);
			else if(configSettings.parentSelection == 2)
				parents = tournamentSelectionParent(population, configSettings);
			else
			{
				for(int x = 0; x < configSettings.numOffspring; x++)
					parents.push_back(uniformRandomSelection(population));
			...
			if(configSettings.survivalSelection == 1)
				...
				fitnessProportionalSurvivalSelection(population, configSettings);
			else
				randomSurvival(population, configSettings);
